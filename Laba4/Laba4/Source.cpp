...
void RandomDataInitialization(double* pAMatrix, double* pBMatrix,
	...
	for (i = 0; i < Rows; i++)
		for (j = 0; j < Columns; j++) {
			pAMatrix[i * Rows + j] = rand() / double(1000);
			pBMatrix[i * Rows + j] = rand() / double(1000);
		}
}
...
void Output(double* pAMatrix, double* pBMatrix, double* pCMatrix,
	int Size) {
	double* pSerialResult; // Result matrix of serial multiplication
	double Accuracy = 1.e-6; // Comparison accuracy
	int equal = 0; // =1, if the matrices are not equal
...
void main(int argc, char* argv[]) {
	...
	if (ProcNum != GridSize * GridSize) {
		...
		Output(pAMatrix, pBMatrix, pCMatrix, Size);
		// Process Termination
